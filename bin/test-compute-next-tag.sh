...
scenario() {
	...
	for release_number in 0 1; do
		git tag "v19.4.1-$release_number"
	done
}
...
expect 'task edit'    v19.5.0-1 "$(merge "$edit_task")"
expect 'template'     v19.5.0-2 "$(merge "$edit_template")"
...
expect 'a task'   v19.4.1-2 "$(merge "$edit_task")"

scenario 'Release numbers past 9'
for release_number in 2 3 4 5 6 7 8 9 10; do
	git tag "v19.4.1-$release_number"
done
expect 'a task' v19.4.1-11 "$(merge "$edit_task")"

scenario 'Reverting to an already released version'
merge "$bump_version" > /dev/null
# The role is now identical to what v19.4.1-1 already published, so there is
# nothing new to release.
expect 'a revert' ''        "$(merge "$revert_version")"

scenario 'Reverting to an already released version, with a change'
merge "$bump_version" > /dev/null
expect 'a revert' v19.4.1-2 "$(merge "$revert_version && $edit_task")"
