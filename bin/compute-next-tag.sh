...
#
# Tags look like `v<GitLab version>-<release>`:
#
# - if defaults/main.yml points at a GitLab version that has never been
#   released, the release counter restarts at 0 (`v19.5.0-0`)
# - otherwise the counter is incremented (`v19.5.0-1`), but only if something
...
if [ -z "$version" ]; then
	echo >&2 "Could not determine the GitLab version from $defaults_path"
	exit 1
fi
